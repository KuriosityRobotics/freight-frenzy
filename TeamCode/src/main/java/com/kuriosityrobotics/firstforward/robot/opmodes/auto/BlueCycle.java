...
public class BlueCycle extends LinearOpMode {
    ...
    public static final Pose BLUE_START_W = Pose.flipped(9.5, 64.5, Math.toRadians(-90)); //start near warehouse
    public static final Pose FIRST_WOBBLE = Pose.flipped(24, 73, Math.toRadians(-110));

    public static final Pose BLUE_WOBBLE_W = Pose.flipped(24, 73, Math.toRadians(-110));
    public static final Pose BLUE_WOBBLE_WALL_POINT = Pose.flipped(7.5, 68, Math.toRadians(180));


    public static final Pose BLUE_BETWEEN_WOBBLE_WALLGAP = Pose.flipped(7, 62.5, Math.toRadians(180));
    public static final Pose BLUE_WALL_GAP = Pose.flipped(7, 46.5, Math.toRadians(180));
    private Pose blueWarehouse = Pose.flipped(8, 33, Math.toRadians(175));
...
//        AutoPaths.waitForVuforia(robot, this, 1250, Pose.flipped(0.25, 0, 0));
        ...
        for (int i = 0; i < numCycles; i++) {
            ...
            intakeVary = Pose.flippedRelative(1.5*i, -4, Math.toRadians(-18));

            AutoPaths.intakePath(robot, blueWarehouse.add(intakeVary), 4500);

//            if (blueWarehouse.y > 7.5)
            if (i % 2 == 1) {
                blueWarehouse = blueWarehouse.add(Pose.flippedRelative(0, -2, 0));
            }
            ...
            if (sawFirst) {
                ...
                PurePursuit backToWobble = new PurePursuit(new WayPoint[]{
                        new WayPoint(robot.getPose()),
                        new WayPoint(BLUE_WALL_GAP.add(Pose.flippedRelative(-1, 0, 0)),  new VelocityLock(40, true)),//,  0.7 * MotionProfile.ROBOT_MAX_VEL, new ArrayList<>()),
                        new WayPoint(BLUE_EXIT_WALLGAP.add(Pose.flippedRelative(-1, 0, 0)), exitActions),//,  0.55 * MotionProfile.ROBOT_MAX_VEL, new ArrayList<>()),
                        new WayPoint(BLUE_EXIT_WALLGAP.x-7,BLUE_EXIT_WALLGAP.y+2),
                        new WayPoint(BLUE_WOBBLE_W.add(Pose.flippedRelative(-1, -3, 0)), 0, robot.outtakeModule.dumpOuttakeAction())
                }, true, 4);
            ...
            if (sawFirst) {
                AutoPaths.waitForVuforia(robot, this, 250, Pose.flippedRelative(0, 0, 0));
            } else {
                sleep(150);
            }
