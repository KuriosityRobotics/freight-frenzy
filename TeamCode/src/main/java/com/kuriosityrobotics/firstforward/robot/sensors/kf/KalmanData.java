...
public class KalmanData {
    ...
    public static KalmanDatum odometryDatum(long time, double globalHeading, double dx, double dy, double dtheta) {
    ...
    public static KalmanDatum gyroDatum(long time, double globalHeading) {
        return new KalmanDatum(
                time,
                Primitive64Matrix.FACTORY.column(globalHeading),
                Primitive64Matrix.FACTORY.column(toRadians(1)),
                Primitive64Matrix.FACTORY.row(0, 0, 1));
    }

    public static KalmanDatum vuforiaDatum(Pose pose) {
        return new KalmanDatum(new double[]{pose.x, pose.y, pose.heading}, new double[]{.04, .04, toRadians(3 * 3)});
    }
}
