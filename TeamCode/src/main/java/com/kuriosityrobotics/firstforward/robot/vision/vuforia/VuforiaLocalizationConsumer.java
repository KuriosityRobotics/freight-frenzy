...
//        rotator.setPosition(ROTATOR_LEFT_POS + rotationAngle * (ROTATOR_RIGHT_POS - ROTATOR_LEFT_POS));
            ...
            for (VuforiaTrackable trackable : this.freightFrenzyTargets) {
                ...
                if (listener.isVisible()) {
                    ...
                    OpenGLMatrix cameraLoc = OpenGLMatrix
                            .translation(SERVO_FORWARD_DISPLACEMENT + CAMERA_VARIABLE_DISPLACEMENT, SERVO_LEFT_DISPLACEMENT, SERVO_VERTICAL_DISPLACEMENT)
                            .multiplied(Orientation.getRotationMatrix(EXTRINSIC, XZY, RADIANS, (float) Math.PI / 2, (float) Math.toRadians(90), 0));
                    ...
                    if (robotLocationTransform != null && vuMarkPoseRelativeToCamera != null) {
                        ...
                        this.detectedPeripheralAngle = (double)rot.secondAngle;
                    } else {
    ...
    public ArrayList<String> logPositionAndDetection() {
        synchronized (this) {
            ...
            if (detectedTrackable == null) {
                ...
                data.add("Peripheral Angle: " + detectedPeripheralAngle);
                data.add("Actual Peripheral Angle: " + Math.toDegrees(Math.abs(angleWrap(Math.toRadians(detectedPeripheralAngle) - Math.PI))));
            }
    ...
    public RealMatrix getLocationRealMatrix() {
        synchronized (this) {
            try {
                if (!robot.visionThread.getManagedCamera().vuforiaActive || detectedData == null) {
                    return null;
                }

                if (Math.abs(angleWrap(Math.toRadians(detectedPeripheralAngle) - Math.PI)) >= Math.toRadians(15)){
                    return null;
                }
